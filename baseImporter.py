...
def rebuild_neurongroup(neurongroup_dict, namespace=None):
    ...
    for init in neurongroup_dict.get("initializers", []):
        ...
        resolved_value = namespace.get(value, value) if isinstance(value, str) else value
        if "index" in init:
            neuron_group[var][init["index"]] = resolved_value
        else:
            neuron_group[var] = resolved_value


    for op in neurongroup_dict.get("run_regularly", []):
        code = compile(op["code"], "<string>", "exec")
